...
// Mirrors the Python CAN preprocessing pipeline exactly.
// --------------------------------------------------------------------------
...
public:
    ...
    static std::vector<float> extractWindow(const std::vector<CanFrame>& window) {
        ...
        for (int i = 0; i < W; i++) {
            ...
            // [15] bitflip_ratio (vs previous frame with same ID)
            float bitflipRatio = 0.0f;
            // We already stored lastPayload before overwriting, so use the
            // captured prev (which is the one before we set it above).
            // Actually we already overwrote it above, so we need a separate map.
            // We'll use a secondary approach: count bitflips inline.
            features[15 * W + col] = bitflipRatio; // Simplified; see note below
        }
